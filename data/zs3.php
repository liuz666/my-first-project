...
if($row = mysql_fetch_assoc($sql)){
	$agent_bid = $row['agent_bid'];
}
//print_r($agent_bid);die;
 ////ajax 查询返回
//print_r($act);die;
 if($act == "query"){ 
	...
	if($keyword != ""){
	  ...
	  if($msg0 == 1){
	  	...
	  	$sql="select * from tgs_brand where brand_name = '{$brandname}' ";
	  	 $res=mysql_query($sql);
	  	if(mysql_num_rows($res)>0){
	  		$array = mysql_fetch_array($res);
	  		$content = $array["brand_accredit"];
	  		$img = $array["brand_pic"];

	  	}

 	$sql="select * from tgs_agent where (weixin='$keyword' or phone='$keyword' or qq='$keyword') and shzt='1' limit 1";
	   // echo "select * from tgs_agent where (agent_bid in($agent_bid)='$keyword' or phone='$keyword' or qq='$keyword') and shzt='1' limit 1";
	   $res=mysql_query($sql);
	   if(mysql_num_rows($res)>0){
		 ...
		 $msg1        = str_replace("{{weixin}}",$weixin,unstrreplace($content)); 
...
$myImage = imagecreatefrompng("upload/".$img);// 证书模版图片文件的路径 
